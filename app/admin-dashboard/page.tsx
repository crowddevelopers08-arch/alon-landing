...
import { Search, Filter, Download, Phone, Mail, Calendar, RefreshCw, Users, FileText, ChevronDown, ChevronUp, Baby, Clock } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function LeadsTable({ 
  ...
  const sortedLeads = [...leads].sort((a, b) => {
    ...
    if (key === 'createdAt' || key === 'updatedAt' || key === 'preferredDate') {
  ...
  const filteredLeads = sortedLeads.filter((lead) => {
    const matchesSearch = 
      ...
      safeString(lead.preferredDate).includes(safeString(searchTerm)) ||
      safeString(lead.preferredTime).includes(safeString(searchTerm))
  ...
  const getBookingStatusBadge = (status?: string) => {
    ...
    return config ? <Badge variant="outline" className={`${config.color} border text-xs`}>{config.label}</Badge> : null
  }
  ...
  const updateBookingStatus = async (leadId: string, bookingStatus: string) => {
    try {
      const response = await fetch(`/api/leads/${leadId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        setLeads(leads.map(lead => 
          lead.id === leadId ? { ...lead, bookingStatus: bookingStatus as any } : lead
        ))
      } else {
      ...
      <Card className="w-full bg-white border-gray-200">
        <CardHeader className="border-b border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle className="text-2xl font-bold text-gray-900">Anlon Leads Management</CardTitle>
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {Object.entries(formStats).length > 0 ? (
              ...
              <Card className="p-4 bg-white border-gray-200 shadow-sm col-span-full">
                <div className="text-center text-gray-500">No leads available</div>
              </Card>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <Select value={treatmentFilter} onValueChange={setTreatmentFilter}>
              ...
              <SelectContent className="bg-white border-gray-300 text-gray-900">
                ...
                <SelectItem value="Initial Consultation" className="focus:bg-gray-100">Initial Consultation</SelectItem>
                <SelectItem value="Hair Fall Treatment" className="focus:bg-gray-100">Hair Fall Treatment</SelectItem>
                <SelectItem value="Regenera Activa" className="focus:bg-gray-100">Regenera Activa</SelectItem>
              </SelectContent>
            </Select>
                            ...
                            <td className="p-4 align-middle">
                              <div className="flex flex-col gap-1">
                                {isSmileBaby ? (
                                  ...
                                  <>
                                    <span className="text-sm font-medium text-gray-900">{lead.treatment || "Consultation"}</span>
                                    {lead.preferredDate && (
                                      <span className="text-xs text-gray-600">Date: {lead.preferredDate}</span>
                                    )}
                                    {lead.preferredTime && (
                                      <span className="text-xs text-gray-600">Time: {lead.preferredTime}</span>
                                    )}
                                            ...
                                            <div className="mt-2">
                                              ...
                                              <div className="flex gap-2 mt-1">
                                                <Button 
                                                  size="sm" 
                                                  variant="outline"
                                                  className="text-xs h-7 bg-yellow-50 border-yellow-200 text-yellow-700 hover:bg-yellow-100"
                                                  onClick={(e) => {
                                                    e.stopPropagation()
                                                    updateBookingStatus(lead.id, 'pending')
                                                  }}
                                                >
                                                  Pending
                                                </Button>
                                                <Button 
                                                  ...
                                                  onClick={(e) => {
                                                    e.stopPropagation()
                                                    updateBookingStatus(lead.id, 'confirmed')
                                                  }}
                                                >
                                                  Confirm
                                                </Button>
                                                <Button 
                                                  ...
                                                  onClick={(e) => {
                                                    e.stopPropagation()
                                                    updateBookingStatus(lead.id, 'completed')
                                                  }}
                                                >
                                                  Complete
                                                </Button>
                                                <Button 
                                                  size="sm" 
                                                  variant="outline"
                                                  className="text-xs h-7 bg-red-50 border-red-200 text-red-700 hover:bg-red-100"
                                                  onClick={(e) => {
                                                    e.stopPropagation()
                                                    updateBookingStatus(lead.id, 'cancelled')
                                                  }}
                                                >
                                                  Cancel
                                                </Button>
