...
const treatments = [
  {
    ...
    name: "HydraFacial",
    subtitle:
      "HydraFacial is an advanced skin treatment designed to deeply cleanse, exfoliate, and hydrate the skin while improving overall skin health and radiance.",
    description:
      "Unlike traditional facials, this multi-step procedure delivers instant glow with no downtime and is suitable for all skin types.",
    idealFor: [
      "Dull & dehydrated skin",
      "Clogged pores & blackheads",
      "Uneven skin tone",
      "Individuals seeking instant glow & hydration",
    ],
    image: "hydr.jpg",
    imageAlt: "HydraFacial advanced deep cleansing and skin hydration treatment",
  },
  {
    id: "acne-treatment",
    number: "02",
    ...
    name: "Acne Treatment",
    subtitle:
      "Acne Treatment is a targeted skin therapy designed to control active acne, reduce inflammation, and prevent future breakouts.",
    description:
      "This treatment works at the root cause of acne to promote clearer, healthier skin over time.",
    idealFor: [
      "Active acne & breakouts",
      "Oily & acne-prone skin",
      "Acne scars & marks",
      "Individuals seeking long-term acne control",
    ],
    image: "treateesed.jpg",
    imageAlt: "Advanced acne control and skin healing treatment",
  },
  {
  ...
  },
  {
    ...
    name: "Chemical Peel",
    subtitle:
      "Chemical Peel is an advanced skin resurfacing treatment designed to remove dead skin cells, improve skin texture, and reduce pigmentation.",
    description:
      "This procedure promotes new skin cell growth, revealing brighter, smoother, and more even-toned skin.",
    idealFor: [
      "Pigmentation & dark spots",
      "Acne marks & scars",
      "Uneven skin tone",
      "Individuals seeking skin brightening & renewal",
    ],
    image: "chemical2.jpg",
    imageAlt: "Chemical peel skin brightening and renewal treatment",
  },
];
...
export default function SkinTreatments() {
  ...
  const current = treatments[activeIndex] ?? treatments[0];
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setActiveIndex((cur) => (cur + 1) % treatments.length);
    }, 3500);
  ...
  const handleTabClick = (id: string) => {
    setPaused(true);
    const nextIndex = treatments.findIndex((t) => t.id === id);
    if (nextIndex !== -1) setActiveIndex(nextIndex);
  };

  const goPrev = () => { setPaused(true); if (activeIndex > 0) setActiveIndex(activeIndex - 1); };
  const goNext = () => { setPaused(true); if (activeIndex < treatments.length - 1) setActiveIndex(activeIndex + 1); };
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <RevealOnScroll direction="up" duration={700}>
            <div style={{ marginBottom: 40, textAlign: "center" }}>
              ...
              <p className="sk-header-desc" style={{ color: "rgba(255,255,255,0.4)", fontSize: 14, maxWidth: 580, lineHeight: 1.7, margin: "0 auto" }}>
                Each designed for a specific Skin concern — all supervised personally by our expert dermatologists.
              </p>
            </div>
          </RevealOnScroll>
          ...
          {!paused && inView && (
            <div className="sk-progress-bar" style={{ marginBottom: 12 }}>
              <div key={current.id} className="sk-progress-fill running" />
            </div>
          )}
          ...
          <RevealOnScroll direction="up" duration={700} delay={150}>
            <div className="sk-tabs-row" style={{ marginBottom: 24 }}>
              ...
              <div className="sk-tabs-scroll" ref={tabsScrollRef}>
                {treatments.map((t) => (
                  <button key={t.id} onClick={() => handleTabClick(t.id)} className={`sk-tab${current.id === t.id ? " active" : ""}`}>
                    <span style={{ display: "block", fontSize: 10, fontWeight: 700, letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 3, color: current.id === t.id ? "#9B7057" : "rgba(255,255,255,0.22)" }}>
                      {t.number}
                    </span>
                    <span style={{ display: "block", fontSize: 13, fontWeight: 600, lineHeight: 1.35, color: current.id === t.id ? "#fff" : "rgba(255,255,255,0.45)" }}>
                      {t.name}
                    </span>
                    <span style={{ display: "block", fontSize: 10, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase", marginTop: 3, color: current.id === t.id ? "rgba(155,112,87,0.8)" : "rgba(255,255,255,0.16)" }}>
                      {t.tag}
                    </span>
                  </button>
                ))}
